  ...
  if user_exists_by_first_name_last_name(f_first_name, f_last_name) is True:
    ...
    if (choice == 'y' or choice == 'Y'):
      ...
      if check_user_credentials(u_user_name, password):
        ...
        update_pending_list(u_user_name, f_user_name, f_first_name,
                            f_last_name)
        #prints user's pending friends list
        print(
            db.collection("users").document(u_user_name).get().to_dict().get(
                "pending_friends_list"))
        update_pending_list(f_user_name, u_user_name, u_first_name,
                            u_last_name)


# Epic 4 Task 4: The next time that that student logs in, they will be informed that
# they have a pending friend request. They can accept / reject the friend request.
# If they accept it, then the person who sent it will be added to their list of friends.
# The person who sent the request will also have this student added to their
# list of friends.
def handle_friend_requests(user_name):
  user_ref = db.collection("users").document(user_name)
  user_data = user_ref.get().to_dict()

  pending_friends_list = user_data.get("pending_friends_list",
                                       {}).get("pending_friends_list", [])

  if not pending_friends_list:
    print("No pending friend requests.")
    return

  print("Pending Friend Requests:")
  for i, friend_request in enumerate(pending_friends_list):
    print(
        f"{i+1}. {friend_request['first_name']} {friend_request['last_name']}")

  choice = input(
      "Enter the number of the friend request you want to accept or reject\
  (0 to go back): ")

  if choice.isdigit():
    choice = int(choice)
    if 0 < choice <= len(pending_friends_list):
      friend_request = pending_friends_list[choice - 1]
      friend_user_name = friend_request["user_name"]
      friend_first_name = friend_request["first_name"]
      friend_last_name = friend_request["last_name"]

      action = input(
          f"Do you want to accept or reject the friend request from \
          {friend_first_name} {friend_last_name}? (a/r): ")

      if action.lower() == 'a':
        update_friends_list(user_name, friend_user_name, friend_first_name,\
                            friend_last_name)
        update_friends_list(friend_user_name, user_name, user_data["first_name"],\
                            user_data["last_name"])
        print(f"{friend_first_name} {friend_last_name} has been added to your\
              friends list.")
      elif action.lower() == 'r':
        print(f"You have rejected the friend request from {friend_first_name}\
              {friend_last_name}.")

      # Remove the processed friend request from the pending list
      updated_pending_list = [friend for friend in pending_friends_list if\
                              friend != friend_request]
      user_ref.update({"pending_friends_list.pending_friends_list"\
...
# Epic 4 Task 5: Each student will be able to generate a list of their pending friend
# requests. A new "show my network" option will be added that will present the user
# with a list of the people that they have connected with (including none). They will
# have the option of disconnecting from any of the listed people. If they disconnect
# from someone, the person will be removed from their list of friends, and they will
# be removed from the other person's list of friends.
def show_network_and_disconnect(user_name):
  user_ref = db.collection("users").document(user_name)
  user_data = user_ref.get().to_dict()

  friends_list = user_data.get("friends_list", {}).get("friends_list", [])

  if not friends_list:
    print("You currently don't have any friends in your network.")
    return

  print("Your Network:")
  for i, friend in enumerate(friends_list):
    print(f"{i+1}. {friend['first_name']} {friend['last_name']}")

  choice = input(
      "Enter the number of the friend you want to disconnect from (0 to go back): "
  )

  if choice.isdigit():
    choice = int(choice)
    if 0 < choice <= len(friends_list):
      friend = friends_list[choice - 1]
      friend_user_name = friend["user_name"]
      friend_first_name = friend["first_name"]
      friend_last_name = friend["last_name"]

      action = input(
          f"Do you want to disconnect from {friend_first_name} {friend_last_name}? (y/n): "
      )

      if action.lower() == 'y':
        # Remove the friend from the user's friends list
        updated_friends_list = [f for f in friends_list if f != friend]
        user_ref.update({"friends_list.friends_list": updated_friends_list})

        # Remove the user from the friend's friends list
        friend_ref = db.collection("users").document(friend_user_name)
        friend_data = friend_ref.get().to_dict()
        updated_friend_friends_list = [
            f for f in friend_data["friends_list"]["friends_list"]
            if f != user_data
        ]
        friend_ref.update(
            {"friends_list.friends_list": updated_friend_friends_list})

        print(
            f"You have disconnected from {friend_first_name} {friend_last_name}."
        )
      elif action.lower() == 'n':
        return
      else:
...
def format_word(word):
  """Formats a word to start with an uppercase letter."""
...
def create_profile(user_name):
  ...
  experience = []
  for i in range(3):
      job_title = input(f"Enter title of job {i+1} (press enter if none): ")
      if not job_title:
          break
      employer = input(f"Enter employer for job {i+1}: ")
      date_started = input(f"Enter start date for job {i+1}: ")
      date_ended = input(f"Enter end date for job {i+1}: ")
      location = input(f"Enter location for job {i+1}: ")
      description = input(f"Enter description for job {i+1}: ")
      experience.append({
          "title": job_title,
          "employer": employer,
  ...
  education = []
  while True:
      school_name = input("Enter name of school (press enter if none): ")
      if not school_name:
          break
  ...
  profile_data = {
      "user_name": user_name,
      "title": format_word(title),
      "major": format_word(major),
      "university": format_word(university),
      "about": about,
...
def display_profile(user_name):
  profile_ref = db.collection("profiles").document(user_name)
  ...
  if profile_data:
      print("\n Your Profile: /n")
      print(f"\nProfile Information for {user_data.get('first_name')} {user_data.get('last_name')}:")
      print(f"Title: {profile_data.get('title')}")
      print(f"Major: {profile_data.get('major')}")
      print(f"University: {profile_data.get('university')}")
      print(f"About: {profile_data.get('about')}")

      print("Experience:")
      for experience in profile_data.get('experience', []):
          print(f"\tTitle: {experience.get('title')}")
          print(f"\tEmployer: {experience.get('employer')}")
          print(f"\tDate Started: {experience.get('date_started')}")
          print(f"\tDate Ended: {experience.get('date_ended')}")
          print(f"\tLocation: {experience.get('location')}")
          print(f"\tDescription: {experience.get('description')}")

      print("Education:")
      for education in profile_data.get('education', []):
          print(f"\tSchool Name: {education.get('school_name')}")
          print(f"\tDegree: {education.get('degree')}")
          print(f"\tYears Attended: {education.get('years_attended')}")
  else:
      print("Profile not found.")

def display_friend_profiles(user_name):
  user_ref = db.collection("users").document(user_name)
  user_data = user_ref.get().to_dict()
  ...
  if user_data:
      ...
      choice = input("Enter the number of the friend whose profile you want to view (0 to go back): ")
      ...
      if choice.isdigit():
          ...
          if 0 < choice <= len(friends_list):
              friend = friends_list[choice - 1]
              friend_user_name = friend["user_name"]
              display_profile(friend_user_name)
          elif choice == 0:
              return
          else:
              print("Invalid choice. Please enter a valid number.")
...
def main_menu():
    ...
    while True:
        ...
        print("8. Show Network of Friends")
        print("9. Create or Update Profile")
        print("10. View Profile")
        print("11. View Friend Profiles")
        print("12. Exit")
        ...
        if choice == '1':
            ...
            username = chooseLogInSignUp()
        elif choice == '4':
            connect_with_people()
            handle_friend_requests(username)
        elif choice == '5':
            create_job()
        elif choice == '6':
            useful_links()
            ...
            show_network_and_disconnect(username)
        elif choice == '9':
            create_profile(username)
        elif choice == '10':
            display_profile(username)
        elif choice == '11':
            display_friend_profiles(username)
        elif choice == '12':
